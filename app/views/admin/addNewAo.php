...
<main class="admin managePage"><!-- remove cont-creator to access all-->
    <section class="card-con">
        <a href="<?php echo URLROOT?>/admin_manage/aoAccounts" class="back"> 
        ...
        <div class="header-con">
            <h1>New Alumni Officer Account</h1>
            <span>Fill the required Details to create account</span>
        </div>
        <form action="<?php echo URLROOT ?>/admin_manage/addNewAo" id="add-new-account" method="POST">
            <div>
                <label for="admin-type" class="outsideLabel">Department:</label>
                <div class="textFieldContainer">
                    <select name="dept_id" id="admin-type" required>
                        <?php foreach($data['departments'] as $dept) : ?>
                        <option value="<?= $dept->id ?>" data-batch="adminType"><?= $dept->department_name?></option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div>
                <label for="contact-no-id" class="outsideLabel">Contact No.:</label>
                <div class="textFieldContainer">
                    <input type="text" name="contact_no" id="contact-no-id" required>
                    <span class="error"><?php echo $data['number_err'] ?></span>
                </div>
            </div>
            <div>
                <label for="facebook-id" class="outsideLabel">Facebook Link:</label>
                <div class="textFieldContainer">
                    <input type="text" name="facebook" id="facebook-id" required>
                    <span class="error"><?php echo $data['facebook_err'] ?></span>
                </div>
            </div>
            <div>
